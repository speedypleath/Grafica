...
void CreateVBO(void)
{
	...
	int k = 0;
	for (float theta = 0.1; theta < 20; theta += 0.1)
	{
		// se foloseste reprezentarea parametrica pentru a genera varfurile (puncte situate pe cerc)
		vf_coord[k++] =
			glm::vec4(3 * cx / sqrt(theta) * cos(theta),
				3 * cx / sqrt(theta) * sin(theta),
				0.0f, 1.0f);
	};
	// culorile varfurilor
...
{
	ProgramId = LoadShaders("../../res/09_01_Shader.vert", "../../res/09_01_Shader.frag");
