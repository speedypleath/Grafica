...
float const U_MIN = -PI, U_MAX = PI, V_MIN = - 40 * PI, V_MAX = 40 * PI;
...
void CreateVBO(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			_index = merid * (NR_PARR + 1) + parr;
			Vertices[_index] = glm::vec4(x_vf, y_vf, z_vf, 1.0);
			Colors[_index] = glm::vec3(0.1f + sinf(u), 0.1f + cosf(v), 0.1f + -1.5 * sinf(u));
			Indices[_index] = _index;
			// indice ptr acelasi varf la parcurgerea paralelelor
			index_aux = parr * (NR_MERID)+merid;
			Indices[(NR_PARR + 1) * NR_MERID + index_aux] = _index;
			...
			if ((parr + 1) % (NR_PARR + 1) != 0) // varful considerat sa nu fie Polul Nord
			{
				...
				int index2 = _index + (NR_PARR + 1); // dreapta lui v, pe meridianul urmator
				...
				Indices[AUX + 4 * _index] = index1;  // unele valori ale lui Indices, corespunzatoare Polului Nord, au valori neadecvate
				Indices[AUX + 4 * _index + 1] = index2;
				Indices[AUX + 4 * _index + 2] = index3;
				Indices[AUX + 4 * _index + 3] = index4;
			}
		}
	};
