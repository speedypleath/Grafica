...
float alpha = 0.0f, yaw = 0.0f, dist = 300.0f;
...
void CreateVBO(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			ind = merid * (NR_PARR + 1) + parr;
			Vertices[ind] = glm::vec4(x_vf, y_vf, z_vf, 1.0);
			Colors[ind] = glm::vec3(0.96 + 0.1f * sinf(u) * cosf(v), 0.71 + 0.1f * cosf(u) * sinf(v), 0.1);
			// Colors[ind] = glm::vec3(1.0f- 0.05 *rand() / RAND_MAX, 1.0f, 0.9 + 0.1f * sinf(u));
			// glm::vec3(0.1f + sinf(u), 0.1f + cosf(v), 0.1f + -1.5 * sinf(u));
			Normals[ind] = glm::vec3(x_vf, y_vf, z_vf);
			Indices[ind] = ind;
			// indice ptr acelasi varf la parcurgerea paralelelor
			ind_aux = parr * (NR_MERID)+merid;
			Indices[(NR_PARR + 1) * NR_MERID + ind_aux] = ind;
			...
			if ((parr + 1) % (NR_PARR + 1) != 0) // varful considerat sa nu fie Polul Nord
			{
				...
				int ind1 = ind; // varful v considerat
				int ind2 = ind + (NR_PARR + 1); // dreapta lui v, pe meridianul urmator
				int ind3 = ind2 + 1;  // dreapta sus fata de v
				int ind4 = ind + 1;  // deasupra lui v, pe acelasi meridian
				if (merid == NR_MERID - 1)  // la ultimul meridian, trebuie revenit la meridianul initial
				{
					ind2 = ind2 % (NR_PARR + 1);
					ind3 = ind3 % (NR_PARR + 1);
				}
				Indices[AUX + 4 * ind] = ind1;  // unele valori ale lui Indices, corespunzatoare Polului Nord, au valori neadecvate
				Indices[AUX + 4 * ind + 1] = ind2;
				Indices[AUX + 4 * ind + 2] = ind3;
				Indices[AUX + 4 * ind + 3] = ind4;
			}
		}
	};
...
{
	ProgramIdv = LoadShaders("../../11_01v_Shader.vert", "../../11_01v_Shader.frag");
	glUseProgram(ProgramIdv);
}
void CreateShadersFragment(void)
{
	ProgramIdf = LoadShaders("../../11_01f_Shader.vert", "../../11_01f_Shader.frag");
